...
| Area | Control | Tests |
| --- | --- | --- |
| Image proxy open-proxy / SSRF | HMAC on `url`, prefs/server enable gate, `validate_proxy_url` blocks loopback/private/link-local/CGNAT/metadata hosts + non-http(s); **redirects disabled**; **DNS resolve re-check** via `validate_resolved_url` at each hop; body capped at 5 MiB | `zoeken-favicons` unit tests; `image_proxy.rs` route tests |
| Favicon proxy SSRF | HMAC on `authority`; `validate_proxy_authority` rejects private hosts including `host:port` / `[::1]:port`; resolve-time IP check on outbound hops; redirects disabled; 1 MiB body cap | `favicon_proxy.rs`, `proxy.rs` |
...
| Cookies | Prefs cookie `HttpOnly` + `SameSite=Lax` + `Secure` when HSTS / https `base_url` / trusted `X-Forwarded-Proto` | prefs cookie tests |
