...
fn load_precompiled_bundle() -> DataBundle {
    ...
    let ahmia_blacklist = include_str!("../data/ahmia_blacklist.txt")
        ...
        .map(str::to_string)
        .collect();
    let doi_resolvers = serde_json::from_str(include_str!("../data/doi_resolvers.json"))
        .expect("embedded DOI resolver data must be valid JSON");
    let autocomplete = serde_json::from_str(include_str!("../data/autocomplete_backends.json"))
        .expect("embedded autocomplete metadata must be valid JSON");
    let limiter_toml = include_str!("../data/limiter.toml").to_string();
    let info_pages = serde_json::from_str(include_str!("../data/info_pages.json"))
        .expect("embedded info page data must be valid JSON");
