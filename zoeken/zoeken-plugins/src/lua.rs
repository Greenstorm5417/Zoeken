...
fn hash_digest(algorithm: &str, input: &str) -> anyhow::Result<String> {
    ...
    match algorithm.to_ascii_lowercase().as_str() {
        "md5" => Ok(digest_hex::<Md5>(bytes)),
        "sha1" => Ok(digest_hex::<Sha1>(bytes)),
        "sha224" => Ok(digest_hex::<Sha224>(bytes)),
        "sha256" => Ok(digest_hex::<Sha256>(bytes)),
        "sha384" => Ok(digest_hex::<Sha384>(bytes)),
        "sha512" => Ok(digest_hex::<Sha512>(bytes)),
