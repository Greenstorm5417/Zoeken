...
async fn get_metrics(router: Router) -> Response {
    router
        .oneshot(
            Request::builder()
                .uri("/metrics")
...
fn metrics_state() -> AppState {
    let mut settings = Settings::defaults();
    settings.general.open_metrics = "secret".to_string();
    AppState::from_search(stub_search()).with_settings(settings)
}
...
#[tokio::test]
async fn information_pages_use_the_requested_locale() {
    let english = get(test_app(), "/info/en/about").await;
    assert_eq!(english.status(), StatusCode::OK);
    assert_eq!(english.headers()[header::CONTENT_LANGUAGE], "en");
    assert!(body_text(english).await.starts_with("About\n\n"));

    let german = get(test_app(), "/info/de-DE/about").await;
    assert_eq!(german.status(), StatusCode::OK);
    assert_eq!(german.headers()[header::CONTENT_LANGUAGE], "de");
    assert!(body_text(german).await.starts_with("Ueber Zoeken\n\n"));
}

#[tokio::test]
async fn information_pages_fall_back_to_the_catalog_default_locale() {
    let response = get(test_app(), "/info/zz/privacy").await;
    assert_eq!(response.status(), StatusCode::OK);
    assert_eq!(response.headers()[header::CONTENT_LANGUAGE], "en");
    assert!(body_text(response).await.starts_with("Privacy\n\n"));
}
...
async fn metrics_renders_prometheus_text_without_handle() {
    let response = get_metrics(app(metrics_state())).await;
...
async fn metrics_renders_injected_handle_exposition() {
    ...
    let router = app(metrics_state().with_metrics_handle(handle));

    let response = get_metrics(router).await;
