...
use crate::static_assets::static_fallback;
...
/// `GET /info/{locale}/{page}` → SPA about page (locale/page ignored).
pub async fn info_page(Path((_locale, _page)): Path<(String, String)>) -> Response {
    Redirect::temporary("/about").into_response()
}
