...
mod tests {
    ...
    async fn status_line_for_configured_version(version: &str) -> String {
        let listener = bind_listener(loopback(0)).await.unwrap();
        let addr = listener.local_addr().unwrap();
        let deployment = DeploymentConfig::default();
        let router = crate::middleware::apply_middleware(
        ...
        let (tx, rx) = tokio::sync::oneshot::channel::<()>();
        let serve_task = tokio::spawn(async move {
            serve_with_shutdown(listener, router, Duration::from_secs(1), async move {
                let _ = rx.await;
            })
            .await
        });
    ...
    #[tokio::test]
    async fn configured_http_version_is_serialized_on_the_wire() {
        assert_eq!(
            status_line_for_configured_version("1.0").await,
            "HTTP/1.0 200 OK"
        );
        assert_eq!(
            status_line_for_configured_version("1.1").await,
            "HTTP/1.1 200 OK"
        );
    }
}
