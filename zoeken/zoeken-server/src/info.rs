...
pub async fn config(State(state): State<Arc<AppState>>) -> Response {
    let response = ConfigResponse {
        ...
        autocomplete_backends: zoeken_autocomplete::BACKEND_NAMES
            .iter()
        ...
        doi_resolvers: state.settings.doi_resolvers.keys().cloned().collect(),
...
mod tests {
    ...
    async fn config_uses_boot_settings() {
        ...
        let app = app(AppState::from_boot(boot));
        let response = app
            .oneshot(
                Request::builder()
                    .uri("/config")
    ...
    #[tokio::test]
    async fn metrics_renders_empty_exposition_without_handle() {
        let response = get("/metrics").await;
        assert_eq!(response.status(), StatusCode::OK);
        assert_eq!(content_type(&response), METRICS_CONTENT_TYPE);
        // Empty exposition (no handle), but a valid text/plain body.
