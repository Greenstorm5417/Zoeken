...
pub async fn preferences_post(
    ...
    let set_cookie = format!(
        "{PREFERENCES_COOKIE}={encoded}; Path=/; Max-Age={COOKIE_MAX_AGE_SECS}; SameSite=Lax; HttpOnly{secure}"
    );
...
pub async fn clear_cookies(State(state): State<Arc<AppState>>, headers: HeaderMap) -> Response {
    ...
    let mut names: Vec<&str> = headers
        ...
        .collect();
    names.push(PREFERENCES_COOKIE);
    names.sort_unstable();
    names.dedup();

    let mut response = Redirect::to("/").into_response();
    for name in names {
        let value = format!("{name}=; Path=/; Max-Age=0; SameSite=Lax; HttpOnly{secure}");
    ...
    async fn clear_cookies_emits_expiring_set_cookie() {
        ...
        let response = app
            .oneshot(
                Request::builder()
                    .uri("/clear_cookies")
                    .header(header::COOKIE, "preferences=old; session=old")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
