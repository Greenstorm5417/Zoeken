...
def parse_rust_routes() -> list[dict[str, Any]]:
    ...
    stubs = ROOT / "zoeken" / "zoeken-server" / "src" / "stubs.rs"
    ...
    if stubs.is_file() and "client_css_or_static" in stubs.read_text(encoding="utf-8"):
...
def data_asset_notes(asset: str, status: str) -> str:
    ...
    if asset in {"limiter config", "info pages"}:
        return "not loaded by DataBundle yet"
